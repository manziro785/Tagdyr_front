...
import { KNOWLEDGE_CARDS } from "../content/cards";
import {
  applyTimeSkip,
  computeLifeIndex,
  recoverAfterYears,
...
function requireLife(db: GuestDb, id: string): StoredLife {
  ...
  if (!life) throw new Error("Жизнь не найдена (гостевой режим)");
  return life;
}

export const localApi: GameApi = {
  async listLives() {
    return loadDb()
  ...
  async createLife(req) {
    ...
    if (db.lives.some((l) => l.slotIndex === req.slotIndex && l.status !== "archived")) {
      throw new Error(`Слот ${req.slotIndex + 1} уже занят`);
    }
    const character = getCharacter(req.characterId);
    if (!character) throw new Error("Неизвестный персонаж");
  ...
  async completeSeason(lifeId, seasonNumber, body): Promise<CompleteSeasonResponse> {
    ...
    if (life.status !== "active") throw new Error("Жизнь уже завершена");
    if (seasonNumber !== life.currentSeason) {
      throw new Error(`Сезон не совпадает: ожидается ${life.currentSeason}`);
    }
  ...
  async finishLife(lifeId) {
    ...
    if (life.status !== "finished") {
      throw new Error("Сначала нужно дожить последний сезон");
    }

    const code = resolveEnding(life.stats, life.flags, life.debts);
    const ending = getEnding(code);
    if (!ending) throw new Error(`Нет контента концовки "${code}"`);
  ...
  async getCards() {
    const db = loadDb();
    const items = KNOWLEDGE_CARDS.map((c) => ({
      ...c,
